...
function getDayName(date) {
  ...
  const dayOfWeek = daysOfWeek[dateString.getUTCDay()];
...
function getNextFriday(date) {
  ...
  const currentDayOfWeek = resultDate.getUTCDay();
  const daysUntilNextFriday =
    currentDayOfWeek === 5 ? 7 : (5 - currentDayOfWeek + 7) % 7;
  resultDate.setUTCDate(resultDate.getUTCDate() + daysUntilNextFriday);
...
function formatDate(date) {
  ...
  const formattedString = `${month}/${day}/${year}, ${hours % 12 || 12}:${minutes.toString().padStart(2, 0)}:${seconds.toString().padStart(2, 0)} ${amPm}`;
...
function getWeekNumberByDate(date) {
  ...
  const weekNumber = Math.ceil((startDate.getUTCDay() + days + 1) / 7);
  return weekNumber;
}
